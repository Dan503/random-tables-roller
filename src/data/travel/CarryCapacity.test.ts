...
describe('1 creature', () => {
	test('20 strength - large - no sled', () => {
		...
		expect(
			generateCarryCapacity({ strengthScore: 20, isLarge: true, isSled: false })
	...
	test('20 strength - large - with sled', () => {
		...
		expect(
			generateCarryCapacity({ strengthScore: 20, isLarge: true, isSled: true })
		).to.equal(expectation)
	})
	...
	test('16 strength - large - no sled', () => {
		...
		expect(
			generateCarryCapacity({ strengthScore: 16, isLarge: true, isSled: false })
		).to.equal(expectation)
	})
	...
	test('16 strength - large - with sled', () => {
		...
		expect(
			generateCarryCapacity({ strengthScore: 16, isLarge: true, isSled: true })
		).to.equal(expectation)
	})
})
